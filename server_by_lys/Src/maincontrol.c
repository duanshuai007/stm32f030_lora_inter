...
static bool generalRespInfo(DeviceNode *pdev, T_Resp_Info *pRespInfo)
{
  ...
  case CMD_MOTOR_DOWN:
    ...
    pRespInfo->resp_data.identify = pdev->u32Identify;
    break;
...
*   向F405发送通知消息
*/
void UartSendMSGToF405(uint8_t status, uint16_t id, uint32_t identify, uint8_t *version)
{
  UartModule *um = gUartMx;
  T_Resp_Info resp;
  ...
  case NORMAL_UNKNOWN_ERROR:
    resp.resp_data.identify = identify;
    break;
  
  case GET_RUN_BIN_FIN:
    memcpy(resp.resp_data.version, version, VERSION_LEN);
    break;
  default:
    break;
  }
...
void F405Task(void)
{
  ...
  if ( DataPoolGetNumByte(dp, (uint8_t *)(&cmd), sizeof(T_Control_Cmd)) ) {
    ...
    if (enable_communication == true) {
      ...
      else if(CMD_UPDATE == cmd.action)
      {
        ...
        if(0 != strcmp((char *)stBootConfig.ucFWVer, (char *)cmd.cmd_identify.verName))
        {
          stBootConfig.ucBootType = TYPE_UART2;
          stBootConfig.ucTryTimes = 0;
          stBootConfig.ucBootSeccessFlg = 0;
          strcpy((char *)stBootConfig.ucNewFWVer, (char *)cmd.cmd_identify.verName);
        ...
        {
          UartSendMSGToF405(GET_RUN_BIN_FIN, 0, 0, CURRENT_FIRMWARE_VERSION);
  ...
  #if 0
        //UartSendMSGToF405(NORMAL_BUSY, 0, cmd.cmd_identify.identify, NULL);
  #else
        if (SendCMDToList(cmd.endpointId, cmd.action, cmd.cmd_identify.identify)  != true) {
