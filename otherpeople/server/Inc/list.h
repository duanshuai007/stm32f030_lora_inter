...
typedef struct DeviceNode {
  ...
  uint32_t u32LastTime;     //最后接收到终端信息的时间
  
  uint32_t u32HeartTime;     //最后接收到终端信息的时间,用来做发心跳包的起始时间

  uint32_t u32ActionStartTime; //抬起和落下收到中间状态的时间

  uint32_t u32CmdStartTime;    //发送命令的开始时间
  ...
  uint8_t u8EpStatus;       //地锁状态，前倾，后倾，直立，卧倒
  uint8_t u8HaveCarFlg;       //地锁上是否有车

  bool DeviceOnLine;        //设备在线状态

  uint32_t u32Data;
  
  uint8_t u8AbnormalRESP;           //地锁异常通知
  uint8_t u8HeartRESP;           //心跳通知
  uint8_t u8CarCheckRESP;           //超声波检测通知
...
//将list中的所有device写入FLASH，重新写FLASH时需要
